...
/*		for ( int j = 0; j < mList.size(); j++ ) {
	...
	protected IWord nextBasicLatin( int c ) throws IOException 
	{
		...
		ctrlMask &= ~START_SS_MASK;			//reset the secondary segment mask.
		...
		if ( ch == -1 || _wspace ) 
		{
			...
			w.setPartSpeech(IWord.EN_POSPEECH);
			if ( ssseg ) ctrlMask |= START_SS_MASK;
			return w;
		}
			...
			char[] chars, int index ) throws IOException 
	{
		...
		ctrlMask &= ~CHECK_CF_MASK;		//reset the fraction check mask.
